...
const ParentDashboard: React.FC = () => {
  ...
  const [progressData, setProgressData] = useState<
    { name: string; progress: number; engagement: number }[]
  >([]);
  ...
  useEffect(() => {
    const fetchChildData = async () => {
      ...
      try {
        ...
        // For parent users with dummy authentication, use mock data
        if (localStorage.getItem("token")?.startsWith("dummy_parent_token_")) {
          ...
          const mockSubjects: Subject[] = [
            ...
            { name: "English", grade: "A", progress: 92, teacher: "Ms. Davis" },
            { name: "History", grade: "B", progress: 74, teacher: "Mr. Brown" },
          ];
          ...
          const mockActivities: Activity[] = [
            {
              type: "assignment",
              subject: "Math",
              description: "Completed algebra homework",
              score: 95,
              date: "2024-01-15",
            },
            {
              type: "quiz",
              subject: "Science",
              description: "Physics quiz on motion",
              score: 88,
              date: "2024-01-14",
            },
            {
          ...
          const mockAlerts: Alert[] = [
            ...
            {
              type: "grade",
              message: "Science grade improved by 10%",
              priority: "medium",
            },
          ];

          const mockProgressData = [
            { name: "Jan", progress: 70, engagement: 75 },
            { name: "Feb", progress: 73, engagement: 78 },
            { name: "Mar", progress: 78, engagement: 82 },
            { name: "Apr", progress: 82, engagement: 85 },
            { name: "May", progress: 85, engagement: 88 },
        ...
        // Original backend integration code for real parent users
        ...
        const child = allStudents.find(
          (student: any) => student._id === childId
        );
        ...
        if (enrolledCourseIds.length > 0) {
          ...
          const coursePromises = enrolledCourseIds.map((id: string) =>
        ...
        const activityList: Activity[] = (child.achievements || []).map(
          (ach: any) => ({
            type: "achievement",
            subject: "General",
            description: ach,
        ...
        if (child.learningRate < 50) {
          alertList.push({
            type: "attention",
            message: "Child learning rate is low",
            priority: "high",
          });
        }
